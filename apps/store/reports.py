...
def today_summary(store_id: int, *, start, end, prev_start) -> dict:
    """Cashing up: one screen, the shift that is ending.

    Deliberately NOT the 30-day report with a shorter window. An owner closing
    up asks a different set of questions from an owner reviewing a month — how
    much is in the drawer, how much of it was points rather than cash, was it
    busier or quieter than yesterday, what did we actually sell, and is
    anything still sitting open on the board. Everything here answers one of
    those; nothing here is a trend.
    """
    zero = Value(Decimal("0"), output_field=DEC)
    sign = Case(
        When(is_return=True, then=Value(-1)),
    ...
    agg = sales.aggregate(
        ...
        tickets=Count("id", filter=Q(is_return=False)),
        returns=Count("id", filter=Q(is_return=True)),
        returns_value=Coalesce(Sum("discounted_total", filter=Q(is_return=True)), zero),
        # What the drawer should hold: points are not cash.
        beans_spent=Coalesce(Sum("beans_spent"), Value(0)),
        with_customer=Count("id", filter=Q(is_return=False, customer__isnull=False)),
    )
    cups = items.aggregate(n=Coalesce(Sum(_signed(F("quantity"))), zero))["n"]
    tickets = agg["tickets"] or 0
    per_ils = int(getattr(settings, "POINTS_PER_ILS", 10)) or 10
    points_value = (Decimal(int(agg["beans_spent"] or 0)) / per_ils).quantize(
        Decimal("0.01")
    )
    revenue = agg["revenue"] or Decimal("0")

    # Yesterday, same length of window, for the one comparison that matters.
    prev = (
        models.Sale.objects.for_pharmacy(store_id)
        .filter(created_at__gte=prev_start, created_at__lt=start)
        .aggregate(revenue=Coalesce(Sum(signed_total), zero))["revenue"]
    )
    ...
    orders = models.Order.objects.for_pharmacy(store_id).filter(
        created_at__gte=start, created_at__lt=end
    )
    order_agg = orders.aggregate(
        placed=Count("id"),
        collected=Count("id", filter=Q(status=models.Order.Status.COLLECTED)),
        cancelled=Count("id", filter=Q(status=models.Order.Status.CANCELLED)),
        still_open=Count("id", filter=Q(status__in=models.Order.OPEN_STATUSES)),
    )

    ledger = models.BeanLedger.objects.for_pharmacy(store_id).filter(
        created_at__gte=start, created_at__lt=end
    )
    pts = ledger.aggregate(
        earned=Coalesce(Sum("delta", filter=Q(delta__gt=0)), Value(0)),
        spent=Coalesce(Sum("delta", filter=Q(delta__lt=0)), Value(0)),
    )
    ...
    return {
        ...
        "end": end.isoformat(),
        "revenue": str(revenue),
        "previous_revenue": str(prev or Decimal("0")),
        "tickets": tickets,
        "returns": agg["returns"],
        "returns_value": str(agg["returns_value"]),
        "cups": str(cups),
        "avg_ticket": str(
            (revenue / tickets).quantize(Decimal("0.01")) if tickets else Decimal("0.00")
        ),
        # The split the drawer actually needs: the till took `cash`, the rest
        # was paid for with points that were earned earlier.
        "cash": str(revenue),
        "points_value": str(points_value),
        "points_spent": int(agg["beans_spent"] or 0),
        "identified_share": (
            round(100 * (agg["with_customer"] or 0) / tickets) if tickets else 0
        ),
        "by_hour": by_hour,
        "top": top,
        "orders": order_agg,
        "points": {
            "earned": int(pts["earned"] or 0),
            "spent": -int(pts["spent"] or 0),
        },
    }
